# Run 20260923-175052-coconuttree-1-500-dac-32-samp-30-ms-skip2

- **Captured:** 2026-09-23T17:50:52
- **Code version:** `27566a7-dirty`
- **Label:** coconuttree #1 500 dac 32 samp 30 ms - skip2

## Conditions

| Field | Value |
|---|---|
| medium | saline tank |
| saline_g_per_l | _not recorded_ |
| fill_depth_mm | _not recorded_ |
| water_temp_c | _not recorded_ |
| electrode_protrusion_mm | _not recorded_ |
| grounding | unknown |
| tank_contents | _not recorded_ |
| target_description | _not recorded_ |
| electrode_map | E1 = marked nail, clockwise viewed from above |
| specimen_id | _not recorded_ |
| circumference_mm | 14.0 |
| thickness_mm | 60.0 |
| major_diameter_mm | _not recorded_ |
| minor_diameter_mm | 104.0 |
| operator | _not recorded_ |
| notes | _not recorded_ |

## Instrument settings

| Setting | Value |
|---|---|
| port | COM12 |
| baud | 115200 |
| pattern | skip-2 |
| current_range | high |
| dac | 500 |
| settle_ms | 30 |
| samples | 32 |
| warmup_frames | 5 |
| target_warmup_frames | 5 |
| baseline_frames | 5 |
| frames | 10 |
| diameter_cm | None |
| log_dir | phase3a_logs |
| log_enabled | True |
| allow_unstable_baseline | False |
| lenient_quality | False |
| filter_pairs | False |
| electrode_offset | 0 |
| electrode_reversed | False |
| self_test_frames | 3 |
| expected_shunt_ohms | None |

## Incomplete

These were not recorded at capture time and cannot be recovered later:

- grounding is not recorded (floating or grounded?)
