...
void emitFrameWithInjectionDistance(uint8_t injectionDistance) {
  Serial.println("FRAME:");

  for (uint8_t iSrc = 0; iSrc < ELECTRODE_COUNT; ++iSrc) {
    const uint8_t iRet = wrapElectrode(iSrc + injectionDistance);
    ...
    for (uint8_t vp = 0; vp < ELECTRODE_COUNT; ++vp) {
      ...
      if (electrodeOverlaps(vp, vn, iSrc, iRet)) {
        continue;
      }

      selectMux(MUX_VP, vp);
      selectMux(MUX_VN, vn);
      const float mv = readDifferentialMv();
      printMeasurement(iSrc, iRet, vp, vn, mv);
    }
  }
...
void emitFrame() {
#ifdef PHASE3A_DEFAULT_OPPOSITE
  emitFrameWithInjectionDistance(ELECTRODE_COUNT / 2);
#else
  emitFrameWithInjectionDistance(1);
#endif
}

void emitOppositeFrame() {
  emitFrameWithInjectionDistance(ELECTRODE_COUNT / 2);
}
...
void printHelp() {
  Serial.println();
  Serial.println("ESP32-S3 Phase 3A 12-Electrode ERT Scanner");
  Serial.println("Commands:");
  Serial.println("  s          - single default full frame");
  Serial.println("  o          - single opposite-drive frame");
  Serial.println("  g          - continuous frames on");
  Serial.println("  x          - continuous frames off");
  Serial.println("  p<number>  - set MCP4725 raw DAC code");
  Serial.println("  t<number>  - set mux settle time in ms");
  Serial.println("  r<number>  - set frame period in ms");
  ...
  Serial.print("Frame period ms: ");
  Serial.println(framePeriodMs);
  Serial.println();
}
...
void handleCommand(String line) {
  ...
  switch (command) {
    ...
    case 'g':
      continuousMode = true;
      Serial.println("[Mode] continuous enabled");
      break;
    case 'x':
      continuousMode = false;
      disableAllMuxes();
      Serial.println("[Mode] continuous disabled");
      break;
    case 'p':
      updateDac(value);
      break;
    case 't':
      ...
      break;
    case 'r':
      framePeriodMs = max<uint16_t>(value, 100);
      Serial.print("[Timing] frame period ms set to ");
      Serial.println(framePeriodMs);
      break;
    case 'h':
      printHelp();
      break;
    default:
      Serial.println("[Error] unknown command, use h");
      break;
  }
}
